...
class PrefixListView(generic.ObjectListView):
    queryset = Prefix.objects.filter(_depth=0)
    filterset = filtersets.PrefixFilterSet
...
class IPRangeIPAddressesView(generic.ObjectChildrenView):
    ...
    def get_children(self, request, parent):
        ...
        ip_range_ips = IPAddress.objects.all()

        start = ipaddress.ip_network((parent.start_address), strict=False)
        
        end = ipaddress.ip_network((parent.end_address), strict=False)  
        ...
        for ip_address_obj in ip_range_ips:
            ip_address = ipaddress.ip_network(ip_address_obj.address, strict=False)
            # Check if the IP address is within the IP range
            if start.network_address <= ip_address.network_address <= end.broadcast_address:
                count += 1
                # Append only the IP address string to the list
                ips_within_range.append(str(ip_address_obj.address))


        # Convert the list of IP address strings to a queryset
        queryset = IPAddress.objects.filter(
            Q(address__in=ips_within_range)
        )
...
# class IPAM_RegionListView(generic.ObjectListView):
#     queryset = IPAM_Region.objects.all()
#     filterset = filtersets.IPAM_RegionFilterSet
#     filterset_form = forms.IPAM_RegionFilterForm
#     table = tables.IPAM_RegionTable


# @register_model_view(IPAM_Region)
# class IPAM_RegionView(generic.ObjectView):
#     queryset = IPAM_Region.objects.all()

#     def get_extra_context(self, request, instance):
#         related_models = (
#             (Prefix.objects.restrict(request.user, 'view').filter(ipam_region=instance), 'ipam_region_id'),
#             (IPRange.objects.restrict(request.user, 'view').filter(ipam_region=instance), 'ipam_region_id'),
#             (VLAN.objects.restrict(request.user, 'view').filter(ipam_region=instance), 'ipam_region_id'),
#         )
...
# @register_model_view(IPAM_Region, 'edit')
# class IPAM_RegionEditView(generic.ObjectEditView):
#     queryset = IPAM_Region.objects.all()
#     form = forms.IPAM_RegionForm


# @register_model_view(IPAM_Region, 'delete')
# class IPAM_RegionDeleteView(generic.ObjectDeleteView):
#     queryset = IPAM_Region.objects.all()    


# class IPAM_RegionBulkImportView(generic.BulkImportView):
#     queryset = IPAM_Region.objects.all()
#     model_form = forms.IPAM_RegionImportForm


# class IPAM_RegionBulkEditView(generic.BulkEditView):
#     queryset = IPAM_Region.objects.all()    
#     filterset = filtersets.IPAM_RegionFilterSet
#     table = tables.IPAM_RegionTable
#     form = forms.IPAM_RegionBulkEditForm


# class IPAM_RegionBulkDeleteView(generic.BulkDeleteView):
#     queryset = IPAM_Region.objects.all()
#     filterset = filtersets.IPAM_RegionFilterSet
#     table = tables.IPAM_RegionTable
...
#             pk=instance.pk
#         ).prefetch_related(
#             'site', 'role', 'ipam_region', 'tenant', 'vlan',
#         )
#         duplicate_prefix_table = tables.PrefixTable(
#             list(duplicate_prefixes),
#             exclude=('vrf', 'utilization'),
...
#     def get_children(self, request, parent):
#         return parent.get_child_prefixes().restrict(request.user, 'view').prefetch_related(
#             'site', 'vrf', 'vlan', 'role', 'ipam_region', 'tenant', 'tenant__group'
#         )
...
#             vrf=instance.vrf
#         ).prefetch_related(
#             'site', 'role', 'ipam_region', 'tenant', 'vlan', 'role'
...
#     # def get_count(self, request, parent):
