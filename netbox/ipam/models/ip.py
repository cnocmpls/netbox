...
class Prefix(GetAvailablePrefixesMixin, PrimaryModel):
    ...
    region = models.ForeignKey(
        ...
        blank=True,
        null=True,
        help_text=_('The region of this prefix')
    )
    ...
    area_network = models.CharField(
        max_length=10,
        choices = PrefixAreaNetworkChoices,
        verbose_name=_('area network'),
        help_text=_('The area network this prefix is part of'),
        blank=True,
        null=True
    )
    ...
    clone_fields = (
        'region', 'site', 'vrf', 'tenant', 'vlan', 'status', 'role', 'is_pool', 'mark_utilized', 'description' ,'area_network',
    )
...
class IPRange(PrimaryModel):
    ...
    def get_child_ips(self):
        ...
        """
        return IPAddress.objects.filter(Q(
    ...
    def utilization(self):
        ...
        # Initialize the count of IP addresses within the range
        ip_addresses_count = 0

        ip_addr = IPAddress.objects.all()


        for ip_address in ip_addr:
            if self.start_address.ip <= ip_address.address.ip <= self.end_address.ip:

                subnet = ip_address

                def subnet_size_ipv4(subnet):
                    network = ipaddress.IPv4Network(subnet)
        
                    return network.num_addresses
                    

                size = subnet_size_ipv4(subnet)
                ip_addresses_count = ip_addresses_count + size


        total_ips = int(self.end_address.ip) - int(self.start_address.ip) + 1


        utiliation_percentage = ((ip_addresses_count / total_ips) * 100)

        # print("Utiliation percentage:", utiliation_percentage)
        
        return utiliation_percentage
    ...
    def get_count(self):
        ...
        start = ipaddress.ip_network((self.start_address), strict=False)
        
        end = ipaddress.ip_network((self.end_address), strict=False)  
...
class IPAddress(PrimaryModel):
    ...
    site_group = models.ForeignKey(
        ...
        blank=True,
        null=True,
        verbose_name=_('Site Group/Territory')
    )
    tenant = models.ForeignKey(
        to='tenancy.Tenant',
        on_delete=models.PROTECT,
...
# class IPAM_Region(OrganizationalModel):

#     """
#     A Region represents the  region of a Prefix for example, "ER, " "NR, " "SR, " "WR"
#     """
...
#     def get_absolute_url(self):
#         return reverse('ipam:ipam_region', args=[self.pk])
...
#     clone_fields = (
#         'site', 'vrf', 'tenant', 'vlan', 'status', 'role', 'ipam_region', 'is_pool', 'mark_utilized', 'description',
#     )
...
#         blank=True,
#         null=True,
#         help_text=_('The region of this range')
#     )
#     mark_utilized = models.BooleanField(
#         verbose_name=_('mark utilized'),
#         default=False,
...
#     clone_fields = (
#         'vrf', 'tenant', 'status', 'role', 'ipam_region', 'description',
#     )
...
#         blank=True,
#         null=True,
#         help_text=_('The region of this IP')
#     )
#     assigned_object_type = models.ForeignKey(
#         to='contenttypes.ContentType',
#         limit_choices_to=IPADDRESS_ASSIGNMENT_MODELS,
...
#     clone_fields = (
#         'vrf', 'tenant', 'status', 'role', 'ipam_region', 'dns_name', 'description',
#     )
