...
class RegionView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        related_models = (
            ...
            (Prefix.objects.restrict(request.user, 'view').filter(region__in=regions), 'region_id'),
            (IPAddress.objects.restrict(request.user, 'view').filter(region__in=regions), 'region_id'),
        )

        # print("Related models:", related_models)
